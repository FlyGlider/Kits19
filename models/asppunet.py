...
class ASPPUNet(nn.Module):
    ...
    def __init__(self, in_ch=1, base_num_features=30, num_classes=3,
        ...
        for u in range(self.num_pool):
            ...
            # psp
            if u < self.num_pool - 2:
                self.aspp_blocks.append(ASPPModule(in_ch=ch_from_skip, inner_ch=ch_from_skip // 5, out_ch=ch_from_skip,
                                                   norm_type=norm_type, dilations=(2, 4, 6), mode=mode))
        ...
        self.aspp_blocks = nn.ModuleList(self.aspp_blocks)
    ...
    def forward(self, x):
        ...
        for u in range(self.num_pool):
            x = self.tu[u](x)
            if u > self.num_pool - 3:
                x = x + skips[-(u + 1)]
            else:
                x = x + self.aspp_blocks[u](skips[-(u + 1)])
...
if __name__ == '__main__':
    tmp = torch.randn(2, 1, 320, 320).cuda()
    model = ASPPUNet(in_ch=1, base_num_features=30, num_classes=3, norm_type='batch', nonlin_type='relu', pool_type='max',
                     pool_layer_kernel_sizes=[(2, 2), (2, 2), (2, 2), (2, 2)], deep_supervision=True, mode='2D').cuda()
    output = model(tmp)
    plt.imshow(output[0].detach().cpu().numpy()[0, 0])
    print('')
