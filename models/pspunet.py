    ...
    def __init__(self, in_ch=1, base_num_features=30, num_classes=3,
        ...
        for u in range(self.num_pool):
            ...
            if u < self.num_pool - 2:
                self.psp_blocks.append(PSPModule(in_ch=ch_from_skip, out_ch=ch_from_skip,
                                                 norm_type=norm_type, sizes=(1, 2, 3, 6), mode=mode))
        ...
        self.psp_blocks = nn.ModuleList(self.psp_blocks)
    ...
    def forward(self, x):
        ...
        for u in range(self.num_pool):
            ...
            if u > self.num_pool - 3:
                ...
                x = x + self.psp_blocks[u](skips[-(u + 1)])
...
if __name__ == '__main__':
    tmp = torch.randn(2, 1, 64, 64, 32).cuda()
    model = PSPUNet(in_ch=1, base_num_features=30, num_classes=3, norm_type='batch', nonlin_type='relu', pool_type='max',
                    pool_layer_kernel_sizes=[(2, 2, 2), (2, 2, 2), (2, 2, 2)], deep_supervision=False, mode='3D').cuda()
    output = model(tmp)
